...
|---|---|---|---|
| `SW-REGISTRY` | Canonical 92-row registry, pinned canonical resource digest, and strict selector loader | Derive signed row-level results from JUnit/artifact predicates and bind them into proof manifests | `uv run pytest packages/evidence/tests -q`; close only when derived receipts cover the exact required-ID set on the candidate SHA |
...
| `SW-M3-OBSERVED` | Process-local controller issues trace and derives state deltas | Bind controller receipt to extracted source and emitted `TransitionFragment`, then retain exact-SHA release evidence | `uv run pytest adapters/telemetry/opentelemetry/tests tests/integration/observation tests/integration/twin -q` |
