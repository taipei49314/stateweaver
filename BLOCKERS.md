...
| `SW-M5-CHAIN` | Exact retained M4 bytes compile eight `OBSERVED` fragments, receive fresh policy decisions, and retain five actual-ASGI clean-root runs, the matching fixed-mode boundary, four controls, and a distinct bounded six-provider transition/restore/destroy witness | Execute and trace the exact application plan against those retained provider states in one Docker-backed boundary, then explicitly admit the row | `uv run pytest apps/cli/tests/test_materialized_search_qualification.py adapters/environments/docker_compose/tests/test_real_adapter.py tests/integration/compiler -q`; split process/provider witnesses remain prerequisite-only |
